
---
[Link to Wayback Machine](https://web.archive.org/web/20200921121446/https://magic.wizards.com/en/articles/archive/latest-developments/your-wish-my-command-2007-11-16)

[_metadata_:author]:- "Aaron Forsythe"
[_metadata_:description]:- "An elf and a goblin walk into a bar. The elf says, `What happened to Devin?` The goblin says, `He's writing next week's Feature Article, so we get Forsythe again.`"
[_metadata_:generator]:- "Drupal 7 (http://drupal.org)"
[_metadata_:node]:- "623191"
[_metadata_:publish_date]:- "2007-11-16"
[_metadata_:source]:- "div-main-content"
[_metadata_:title]:- "Your Wish Is My Command"
[_metadata_:wayback_capture_timestamp]:- "2020-09-21 12:14:46"
[_metadata_:wayback_raw_url]:- "https://web.archive.org/web/20200921121446id_/https://magic.wizards.com/en/articles/archive/latest-developments/your-wish-my-command-2007-11-16"
[_metadata_:wayback_url]:- "https://magic.wizards.com/en/articles/archive/latest-developments/your-wish-my-command-2007-11-16"
---


Your Wish Is My Command
=======================



 Posted in **Latest Developments**
 on November 16, 2007 






![](https://media.magic.wizards.com/styles/auth_small/public/images/person/authorpic_aaronforsythe.jpg)
By Aaron Forsythe













> 
> *An elf and a goblin walk into a bar.*
> 
> 
> *The elf says, "What happened to Devin?"*
> 
> 
> *The goblin says, "He's writing next week's Feature Article, so we get Forsythe again."*
> 
> 
> 


Hello, again! I have been enjoying three things since I last occupied this post: reading Devin's articles, not writing articles myself, and *Lorwyn*. Sadly, numbers one and two aren't options this week, but I'll try to make the best of it. To make this experience as enjoyable as possible for myself, I'm going to talk about my favorite cycle in *Lorwyn*, the Commands.


As a designer, I really enjoy it when sets contain rare cycles of spells that aren't really in-theme with the rest of what's going on, for two reasons. One, they give the set another "hook" for players to talk about without adding additional complex layers to the set's gameplay, and two, they allow us to make things that might excite players that would otherwise not enjoy that set's particular theme. (Yes, believe it or not, there are players out there that don't like tribal! No, I don't want email explaining why you are one of them! I bet you hate circuses and ice cream, too!)


![Cunning Wish, Beacon of Destruction, and Shining Shoal](https://media.magic.wizards.com/image_legacy_migration/magic/images/mtgcom/fcpics/latest/dl11_cardfan.jpg)Three standout cycles of this ilk are the [Wishes](http://ww2.wizards.com/Gatherer/index.aspx?term=wish&Field_Name=on&setfilter=Judgment&output=Spoiler) from *Judgment*, the [Beacons](http://ww2.wizards.com/Gatherer/index.aspx?term=beacon&Field_Name=on&setfilter=Fifth%20Dawn&output=Spoiler) from F*ifth Daw*n, and the [Shoals](http://ww2.wizards.com/Gatherer/index.aspx?term=shoal&Field_Name=on&setfilter=Betrayers%20of%20Kamigawa&output=Spoiler) from *Betrayers of Kamigawa*. They were hyped to varying degrees—*Judgment* was basically marketed on the backs of the Wishes, whereas the Beacons were much "quieter" in *Fifth Dawn*. Each is a solid, splashy cycle that had impact on the tournament scene, but wasn't beholden to their sets' respective themes of green-white matters, five-color artifacts, and Spirits and Ninjas. They were just great **Magic** cards in a vacuum.


When I was anointed the lead designer of *Lorwyn*, I made sure to carve out slots for such a cycle. I knew that this set in particular was going to be very creature-focused and that I wanted something that would wow the Spikier crowd—you know, those guys that like to [Wrath of God](http://gatherer.wizards.com/Pages/Card/Details.aspx?&name=Wrath%2Bof%2BGod) away creatures as opposed to play them.


The idea for the Command cycle—called the "Choose-Two" cycle in design and development—came from a card I designed as a potential hole-filler for *Dissension*. It was a black-red uncommon called "Rakdos Choose-Two Charm" and it was similar in structure to the cycle in *Lorwyn*—a modal card in which you chose two of four. I'm glad the card wasn't selected to be used in *Dissension* as that would have precluded the *Lorwyn* cycle from existing.


 



![Emerald Charm](http://gatherer.wizards.com/Handlers/Image.ashx?type=card&name=Emerald+Charm)

In general, I love the gameplay of modal cards in **Magic** (although I will admit that many of them add up to hodge-podge spells that are essentially flavorless, which means we shouldn't do them all that often). Modal cards allow you to put cards into your deck with effects that you wouldn't normally include. I know I'd probably never put an instant into a deck that did nothing but make a creature lose flying, but some percentage of the times I played [Emerald Charm](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Emerald+Charm)—usually included in my decks for its ability to destroy global enchantments—I used it to take flying away from something at a key moment.

So my goal was to make cards that were both utility and splashy. I was sure I could do it, but unsure of how the rest of the design team would react to them, so I did a bit of "vigilante design," making them myself and putting them into the file just before the handoff to development. Here's what I handed off:



> 
> White Choose-Two  
> 
> 4WW  
> 
> Sorcery  
> 
> Choose two - Destroy all creatures; or destroy all lands; or destroy all artifacts and enchantments; or each player discards his or her hand.
> 



> 
> Blue Choose-Two  
> 
> 1UUU  
> 
> Instant  
> 
> Choose two - Counter target spell; or return target permanent to its owner's hand; or tap all creatures you don't control; or draw a card.
> 



> 
> Black Choose-Two  
> 
> XBB  
> 
> Sorcery  
> 
> Choose two - Target player loses X life; or target creature gets -X/-X; or return target creature card with converted mana cost X or less in a graveyard to play under your control; or you gain X life.
> 



> 
> Red Choose-Two  
> 
> 3RR  
> 
> Instant  
> 
> Choose two - CARDNAME deals 2 damage to each creature and player; or CARDNAME deals 4 damage to target creature or player; or destroy target land; or choose new targets for target instant or sorcery spell.
> 



> 
> Green Choose-Two  
> 
> 4GG  
> 
> Instant  
> 
> Choose two - Put a 4/4 green Elemental creature token into play; or put four 1/1 green Elf Warrior creature tokens into play; or gain 8 life; or CARDNAME deals 4 damage to each creature with flying.
> 


From here, development went to work.


### Austere Command



> 
> White Choose-Two  
> 
> 4WW  
> 
> Sorcery  
> 
> Choose two - Destroy all creatures; or destroy all lands; or destroy all artifacts and enchantments; or each player discards his or her hand.
> 


The white one was certainly the most ambitious from a design standpoint, not just because of its power but because of what abilities it was trying to give to white. [Wrath of God](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Wrath+of+God) is white, [Purify](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Purify) is white, [Armageddon](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Armageddon) is potentially still white, but [Mind Twist](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Mind+Twist) all players? Does that make any sense? It does to me, as it is a form of "balancing" or mass destruction. "Destroy all \_\_\_\_\_" is white, and to me, "destroying" all the cards in players' hands is a logical extension of that. I'm not sure anyone else bought that argument or if it came off the card just because of how absurdly powerful it was. (Imagine: "I have bigger creatures than you, so I'll destroy all lands and we'll both discard our hands. Good luck!")


The ability that initially replaced the mass [Mind Twist](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Mind+Twist) was [Morningtide](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Morningtide) (the card, not the set), meaning "Remove all cards in graveyards from the game," another mass removal effect of sorts. From there, "[Wrath of God](http://gatherer.wizards.com/Pages/Card/Details.aspx?&name=Wrath%2Bof%2BGod)-ageddon" was still to potent a card, and the land-destroying part was removed, and destroying artifacts and destroying enchantments were split out into their own abilities. The problem there was that the card now had essentially one powerful mode ([Wrath of God](http://gatherer.wizards.com/Pages/Card/Details.aspx?&name=Wrath%2Bof%2BGod)) and three situational ones. To remedy that, the development team ditched the anti-graveyard clause and split the [Wrath of God](http://gatherer.wizards.com/Pages/Card/Details.aspx?&name=Wrath%2Bof%2BGod) ability into a half that destroyed cheap creatures and a half that destroyed expensive ones. Suddenly the card was offering interesting play decisions, as it was possible to destroy all your opponents key permanents and none of your own—[Austere Command](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Austere+Command) is a terrific [Wrath of God](http://gatherer.wizards.com/Pages/Card/Details.aspx?&name=Wrath%2Bof%2BGod) variant.



![Austere Command](http://gatherer.wizards.com/Handlers/Image.ashx?type=card&name=Austere+Command)

### Cryptic Command



> 
> Blue Choose-Two  
> 
> 1UUU  
> 
> Instant  
> 
> Choose two - Counter target spell; or return target permanent to its owner's hand; or tap all creatures you don't control; or draw a card.
> 


The Blue Choose-Two should look familiar—it was printed in a state almost identical to the one submitted ("tap all creatures you don't control" became "tap all creatures opponents control" to be friendlier in team games). But that doesn't mean it didn't undergo a few changes along the way. Our initial impressions after playing with it were that it was too good, although the abilities were reasonable. To solve that, its mana cost was increased to ![](https://web.archive.org/web/20161227130050im_/http://magic.wizards.com/sites/all/modules/custom/wiz_autocard/wiz_manacost/manasymbols/colorless3.gif)![](https://web.archive.org/web/20161115194122im_/http://magic.wizards.com/sites/all/modules/custom/wiz_autocard/wiz_manacost/manasymbols/blue.gif)![](https://web.archive.org/web/20161115194122im_/http://magic.wizards.com/sites/all/modules/custom/wiz_autocard/wiz_manacost/manasymbols/blue.gif) where it stayed for a good chunk of development. Unfortunately, at ![](https://web.archive.org/web/20161227130050im_/http://magic.wizards.com/sites/all/modules/custom/wiz_autocard/wiz_manacost/manasymbols/colorless3.gif)![](https://web.archive.org/web/20161115194122im_/http://magic.wizards.com/sites/all/modules/custom/wiz_autocard/wiz_manacost/manasymbols/blue.gif)![](https://web.archive.org/web/20161115194122im_/http://magic.wizards.com/sites/all/modules/custom/wiz_autocard/wiz_manacost/manasymbols/blue.gif) the card wasn't played much in the FFL. To make it more appealing, the last ability was altered to become "draw two cards"—a change that made the card certainly playable, but much less interesting as every time it was played "draw two cards" was chosen as one of the modes. So it was reverted to draw one again and stayed at five mana.


Toward the end of development, we had a change of heart about the four-mana version being "too good" and decided that it was simply "very good," thus it went back to ![](https://web.archive.org/web/20211201201921im_/https://magic.wizards.com/sites/all/modules/custom/wiz_autocard/wiz_manacost/manasymbols/colorless1.gif)![](https://web.archive.org/web/20161115194122im_/http://magic.wizards.com/sites/all/modules/custom/wiz_autocard/wiz_manacost/manasymbols/blue.gif)![](https://web.archive.org/web/20161115194122im_/http://magic.wizards.com/sites/all/modules/custom/wiz_autocard/wiz_manacost/manasymbols/blue.gif)![](https://web.archive.org/web/20161115194122im_/http://magic.wizards.com/sites/all/modules/custom/wiz_autocard/wiz_manacost/manasymbols/blue.gif). A powerful card, to be sure, but I was happy to see it come back as players' favorite card in the set as a result of our online surveys.



![Cryptic Command](http://gatherer.wizards.com/Handlers/Image.ashx?type=card&name=Cryptic+Command)

### Profane Command



> 
> Black Choose-Two  
> 
> XBB  
> 
> Sorcery  
> 
> Choose two - Target player loses X life; or target creature gets -X/-X; or return target creature card with converted mana cost X or less in a graveyard to play under your control; or you gain X life.
> 


The Black Choose-Two was always an X-spell; I guess I designed one that way because I could, and black was the best choice because as a color it has access to the widest variety of scaling effects. Like the first two, the submitted version felt too powerful initially, mainly because of the life gain element; often times a race was lost because of a four- or five-point increase in life essentially for free on a removal / burn / reanimation spell. All told, the card was strictly better than [Consume Spirit](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Consume+Spirit) by a mile, and that really bugged development as [Consume Spirit](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Consume+Spirit) is a fine constructed card in its own right. To compensate, the first ability was changed to "Each player loses X life" and later the mana cost was increased to ![](https://web.archive.org/web/20160829100442im_/http://magic.wizards.com/sites/all/modules/custom/wiz_autocard/wiz_manacost/manasymbols/x.gif)![](https://web.archive.org/web/20161227195556im_/http://magic.wizards.com/sites/all/modules/custom/wiz_autocard/wiz_manacost/manasymbols/black.gif)![](https://web.archive.org/web/20161227195556im_/http://magic.wizards.com/sites/all/modules/custom/wiz_autocard/wiz_manacost/manasymbols/black.gif)![](https://web.archive.org/web/20161227195556im_/http://magic.wizards.com/sites/all/modules/custom/wiz_autocard/wiz_manacost/manasymbols/black.gif).


With these changes, the card drifted out of Constructed play. As it sat on the sidelines, complaints started coming in that "gain X life" as a mode felt very out of flavor for black. My argument for including it in the first place was that it allowed you to "build" a [Consume Spirit](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Consume+Spirit) on either a creature or player, much as the blue one let you "build" [Dismiss](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Dismiss) and the white one let you "build" [Akroma's Vengeance](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Akroma%27s+Vengeance) (initially). But considering the number of people that didn't like it and didn't see it as fitting, it was a wrong choice. Once the life gain was removed and replaced with the "up to X target creatures gain fear" ability ([Mind Twist](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Mind+Twist) was a contender for, oh, about an hour), the rest of the card was restored to its original functionality, including making a single player lose X life for ![](https://web.archive.org/web/20160829100442im_/http://magic.wizards.com/sites/all/modules/custom/wiz_autocard/wiz_manacost/manasymbols/x.gif)![](https://web.archive.org/web/20161227195556im_/http://magic.wizards.com/sites/all/modules/custom/wiz_autocard/wiz_manacost/manasymbols/black.gif)![](https://web.archive.org/web/20161227195556im_/http://magic.wizards.com/sites/all/modules/custom/wiz_autocard/wiz_manacost/manasymbols/black.gif).



![Profane Command](http://gatherer.wizards.com/Handlers/Image.ashx?type=card&name=Profane+Command)

### Incendiary Command



> 
> Red Choose-Two  
> 
> 3RR  
> 
> Instant  
> 
> Choose two - CARDNAME deals 2 damage to each creature and player; or CARDNAME deals 4 damage to target creature or player; or destroy target land; or choose new targets for target instant or sorcery spell.
> 


Like the white one, the Red Choose-Two was an ambitiously insane design from a power standpoint. End-of-turn [Stone Rain](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Stone+Rain) + [Steam Blast](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Steam+Blast) was absurd versus most tribe decks, and the ability to kill larger creatures with the "deal 4" option often meant your opponent was going to be losing lots of permanents every time this was played. And in a pinch, it was a six-damage burn spell to the opponent's dome for five mana.


Development killed the land destruction part first, replacing it with a spell-copying mode, a la Fork or [Twincast](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Twincast). That change inadvertently created a two-card combo kill: Red Choose-Two and [Twincast](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Twincast) (or [Reiterate](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Reiterate)). All you had to do was wait for an opponent to play an instant or sorcery (or get enough mana to play another one yourself), the play the Choose-Two, choosing to deal 4 to the opponent and copy the instant/sorcery on the stack. From there, you [Twincast](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Twincast) the Choose-Two, dealing 4 again and copying the [Twincast](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Twincast) (you have to choose the same modes when copying). As the [Twincast](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Twincast) never actually gets a change to leave the stack, this combo could kill any number of players from any life total, all at end of turn having done nothing else the entire game. As is typical with such combos, our rogue deckbuilder extraordinaire Steve Warner unleashed this monstrosity on the FFL, and we're grateful he found it.


To solve that problem, the [Fork](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Fork) and [Shunt](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Shunt) modes were stripped off and made into their own card, [Wild Ricochet](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Wild+Ricochet). Land destruction was reinstated, this time hitting only nonbasics, and the "[Winds of Change](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Winds+of+Change)" option was created. (I love this mode; in the true spirit of modal spells, it comes up more often than you'd think when playing the card). The [Pyroclasm](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Pyroclasm) effect was altered to no longer damage players and "instant" became "sorcery" for balance.


That said, the card was still pretty ridiculous as a removal spell and was continuing to blow out our creature decks. Using the same logic that killed [Shard Phoenix](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Shard+Phoenix) from *Tenth Edition*—we didn't want to crush tribe decks before they ever got off the ground—the "deal 4" option was changed to hit just players. Sadly, this change left the card as the worst of the cycle in the end, but something had to end up with that title.



![Incendiary Command](http://gatherer.wizards.com/Handlers/Image.ashx?type=card&name=Incendiary+Command)

### Primal Command



> 
> Green Choose-Two  
> 
> 4GG  
> 
> Instant  
> 
> Choose two - Put a 4/4 green Elemental creature token into play; or put four 1/1 green Elf Warrior creature tokens into play; or gain 8 life; or CARDNAME deals 4 damage to each creature with flying.
> 


The Green Choose-Two was the only one that was scrapped in its entirety by development. The reasons were twofold: One, green had tons of fat creatures and very few spells at rare, and a spell that made a fat creature wasn't what the set needed. Two—and much funnier—the card [Guardian of Cloverdell](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Guardian+of+Cloverdell) already did a lot of what the Choose-Two was doing, including giving you one big creature, a bunch of smaller ones, and some life gain. The card was totally devastating, an instant entwined [One Dozen Eyes](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=One+Dozen+Eyes) that demolished opponents that were foolish enough to attack into someone with six mana untapped.


Mark Gottlieb and I came up with a suite of new abilities for the card one day:



> 
> Choose two - You gain 10 life; or put a +1/+1 counter on each creature you control; or put target noncreature permanent on top of its owner's library; or search your library for a creature card, reveal it, put it into your hand, and shuffle your library.
> 


Development didn't like that this version didn't let you "build" any kind of known spell with the modes, so they chopped out the +1 counter option and added a mode that shuffled a graveyard into a library, and then ordered the abilities so that you could "combo" the modes in two ways. Either you could shuffle your own graveyard in and then tutor, allowing you to essentially get something that was in your graveyard, or you could put an opponent's permanent on top of his or her library and then force them to shuffle it in. For balance reasons, 10 life was dropped to 7, and the card went from a six-mana instant to a five-mana sorcery.


 



![Primal Command](http://gatherer.wizards.com/Handlers/Image.ashx?type=card&name=Primal+Command)

I believe this card is currently being underrated; it gives green a decent answer to planeswalkers or graveyard shenanigans like [Haakon, Stromgald Scourge](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Haakon%2C+Stromgald+Scourge) in a pinch, and the life gain + tutor modes swing many creature fights. Plus the tutor option lets you include single copies of cards like [Riftsweeper](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Riftsweeper) or [Viridian Shaman](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Viridian+Shaman).


Laundry Coleman won Louisiana States this year with a deck featuring [Primal Command](http://gatherer.wizards.com/Pages/Card/Details.aspx?name=Primal+Command); I'm sure he used it to tutor up Crovax quite a few times.








#### Laundry Coleman


##### 






![Download Arena Decklist](https://web.archive.org/web/20211024134741im_/https://magic.wizards.com/sites/all/modules/features/wiz_bean_content_deck_list/icons/decklist_download_arena.png)
![](https://web.archive.org/web/20160726232508im_/http://magic.wizards.com/sites/all/modules/features/wiz_bean_content_deck_list/icons/decklist_download.png)






[Decklist](#none)
[Stats](#none)
[Sample Hand](#none)





Sort by:
OverviewColorCostRarity




##### Planeswalker (3)



3
[Garruk Wildspeaker](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BGarruk%5D+%5BWildspeaker%5D)



##### Creature (14)



4
[Wall of Roots](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BWall%5D+%5Bof%5D+%5BRoots%5D)


3
[Masked Admirers](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BMasked%5D+%5BAdmirers%5D)


4
[Ohran Viper](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BOhran%5D+%5BViper%5D)


1
[Crovax, Ascendant Hero](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BCrovax,%5D+%5BAscendant%5D+%5BHero%5D)


1
[Magus of the Disk](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BMagus%5D+%5Bof%5D+%5Bthe%5D+%5BDisk%5D)


1
[Purity](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BPurity%5D)



##### Sorcery (14)



4
[Into the North](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BInto%5D+%5Bthe%5D+%5BNorth%5D)


3
[Wrath of God](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BWrath%5D+%5Bof%5D+%5BGod%5D)


3
[Harmonize](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BHarmonize%5D)


3
[Primal Command](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BPrimal%5D+%5BCommand%5D)


1
[Austere Command](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BAustere%5D+%5BCommand%5D)



##### Instant (3)



3
[Condemn](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BCondemn%5D)



##### Enchantment (2)



2
[Sacred Mesa](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BSacred%5D+%5BMesa%5D)



##### Land (24)



8
[Snow-Covered Forest](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BSnow-Covered%5D+%5BForest%5D)


6
[Snow-Covered Plains](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BSnow-Covered%5D+%5BPlains%5D)


4
[Scrying Sheets](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BScrying%5D+%5BSheets%5D)


4
[Arctic Flats](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BArctic%5D+%5BFlats%5D)


2
[Mouth of Ronom](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BMouth%5D+%5Bof%5D+%5BRonom%5D)


60 Cards 


##### Sideboard (15)



3
[Seedborn Muse](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BSeedborn%5D+%5BMuse%5D)


2
[Eyes of the Wisent](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BEyes%5D+%5Bof%5D+%5Bthe%5D+%5BWisent%5D)


4
[Spike Feeder](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BSpike%5D+%5BFeeder%5D)


3
[Oblivion Ring](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BOblivion%5D+%5BRing%5D)


3
[Quagnoth](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BQuagnoth%5D)




##### White (15)



1
[Crovax, Ascendant Hero](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BCrovax,%5D+%5BAscendant%5D+%5BHero%5D)


1
[Magus of the Disk](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BMagus%5D+%5Bof%5D+%5Bthe%5D+%5BDisk%5D)


1
[Purity](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BPurity%5D)


3
[Condemn](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BCondemn%5D)


3
[Wrath of God](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BWrath%5D+%5Bof%5D+%5BGod%5D)


2
[Sacred Mesa](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BSacred%5D+%5BMesa%5D)


1
[Austere Command](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BAustere%5D+%5BCommand%5D)


3
[Oblivion Ring](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BOblivion%5D+%5BRing%5D)



##### Green (36)



4
[Wall of Roots](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BWall%5D+%5Bof%5D+%5BRoots%5D)


3
[Masked Admirers](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BMasked%5D+%5BAdmirers%5D)


4
[Ohran Viper](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BOhran%5D+%5BViper%5D)


4
[Into the North](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BInto%5D+%5Bthe%5D+%5BNorth%5D)


3
[Harmonize](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BHarmonize%5D)


3
[Garruk Wildspeaker](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BGarruk%5D+%5BWildspeaker%5D)


3
[Primal Command](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BPrimal%5D+%5BCommand%5D)


3
[Seedborn Muse](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BSeedborn%5D+%5BMuse%5D)


2
[Eyes of the Wisent](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BEyes%5D+%5Bof%5D+%5Bthe%5D+%5BWisent%5D)


4
[Spike Feeder](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BSpike%5D+%5BFeeder%5D)


3
[Quagnoth](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BQuagnoth%5D)



##### Colorless (24)



8
[Snow-Covered Forest](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BSnow-Covered%5D+%5BForest%5D)


6
[Snow-Covered Plains](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BSnow-Covered%5D+%5BPlains%5D)


4
[Scrying Sheets](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BScrying%5D+%5BSheets%5D)


4
[Arctic Flats](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BArctic%5D+%5BFlats%5D)


2
[Mouth of Ronom](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BMouth%5D+%5Bof%5D+%5BRonom%5D)


75 Cards 



##### 1 (3)



3
[Condemn](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BCondemn%5D)



##### 2 (10)



4
[Wall of Roots](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BWall%5D+%5Bof%5D+%5BRoots%5D)


4
[Into the North](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BInto%5D+%5Bthe%5D+%5BNorth%5D)


2
[Eyes of the Wisent](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BEyes%5D+%5Bof%5D+%5Bthe%5D+%5BWisent%5D)



##### 3 (13)



4
[Ohran Viper](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BOhran%5D+%5BViper%5D)


2
[Sacred Mesa](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BSacred%5D+%5BMesa%5D)


4
[Spike Feeder](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BSpike%5D+%5BFeeder%5D)


3
[Oblivion Ring](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BOblivion%5D+%5BRing%5D)



##### 4 (13)



3
[Masked Admirers](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BMasked%5D+%5BAdmirers%5D)


1
[Magus of the Disk](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BMagus%5D+%5Bof%5D+%5Bthe%5D+%5BDisk%5D)


3
[Wrath of God](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BWrath%5D+%5Bof%5D+%5BGod%5D)


3
[Harmonize](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BHarmonize%5D)


3
[Garruk Wildspeaker](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BGarruk%5D+%5BWildspeaker%5D)



##### 5 (6)



3
[Primal Command](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BPrimal%5D+%5BCommand%5D)


3
[Seedborn Muse](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BSeedborn%5D+%5BMuse%5D)



##### 6 (6)



1
[Crovax, Ascendant Hero](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BCrovax,%5D+%5BAscendant%5D+%5BHero%5D)


1
[Purity](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BPurity%5D)


1
[Austere Command](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BAustere%5D+%5BCommand%5D)


3
[Quagnoth](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BQuagnoth%5D)


51 Cards 



##### Common (25)



8
[Snow-Covered Forest](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BSnow-Covered%5D+%5BForest%5D)


6
[Snow-Covered Plains](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BSnow-Covered%5D+%5BPlains%5D)


4
[Wall of Roots](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BWall%5D+%5Bof%5D+%5BRoots%5D)


4
[Into the North](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BInto%5D+%5Bthe%5D+%5BNorth%5D)


3
[Wrath of God](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BWrath%5D+%5Bof%5D+%5BGod%5D)



##### Uncommon (19)



4
[Arctic Flats](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BArctic%5D+%5BFlats%5D)


2
[Mouth of Ronom](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BMouth%5D+%5Bof%5D+%5BRonom%5D)


3
[Condemn](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BCondemn%5D)


3
[Harmonize](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BHarmonize%5D)


4
[Spike Feeder](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BSpike%5D+%5BFeeder%5D)


3
[Oblivion Ring](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BOblivion%5D+%5BRing%5D)



##### Rare (28)



4
[Scrying Sheets](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BScrying%5D+%5BSheets%5D)


3
[Masked Admirers](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BMasked%5D+%5BAdmirers%5D)


4
[Ohran Viper](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BOhran%5D+%5BViper%5D)


1
[Crovax, Ascendant Hero](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BCrovax,%5D+%5BAscendant%5D+%5BHero%5D)


1
[Magus of the Disk](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BMagus%5D+%5Bof%5D+%5Bthe%5D+%5BDisk%5D)


1
[Purity](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BPurity%5D)


2
[Sacred Mesa](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BSacred%5D+%5BMesa%5D)


3
[Primal Command](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BPrimal%5D+%5BCommand%5D)


1
[Austere Command](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BAustere%5D+%5BCommand%5D)


3
[Seedborn Muse](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BSeedborn%5D+%5BMuse%5D)


2
[Eyes of the Wisent](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BEyes%5D+%5Bof%5D+%5Bthe%5D+%5BWisent%5D)


3
[Quagnoth](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BQuagnoth%5D)



##### Mythic (3)



3
[Garruk Wildspeaker](http://gatherer.wizards.com/Pages/Search/Default.aspx?name=+%5BGarruk%5D+%5BWildspeaker%5D)


75 Cards 




![](http://gatherer.wizards.com/Handlers/Image.ashx?type=card&name=Snow-Covered+Forest)


















 

And those are the *Lorwyn* Commands. I'm thrilled with how they turned out—their art is mysterious and beautiful, their power-level is spot-on for high-level play, and their list of options all include a few surprises that can bail you out of oddball situations.


We now return you to your regularly-scheduled Devin Low.


### Last Week's Poll




| **Before reading this sentence, did you realize that you can champion the enchantment Militia's Pride with Thoughtweft Trio?** |
| Yes | 6110 | 70.2% |
| No | 2589 | 29.8% |
| **Total** | **8699** | **100.0%** |

The ability to champion noncreatures was instituted quite late in development. We figured that tribal cards were meant to be equivalent to creatures in as many zones as possible, and that included their ability to be championed, even if the flavor of how that works isn't obvious. I hope that for those of you that selected "no," that moment of discovery was a positive one!


### This Week's Poll




Thank you for voting!


You have already voted!



...

Which are your favorite splashy rare spell cycles? **Choose two**—




|  The [Winds](http://ww2.wizards.com/Gatherer/index.aspx?term=wind%20of%20ascension&Field_Name=on&Field_Rules=on&Field_Type=on&Field_Flavor=on&setfilter=Prophecy) from *Prophecy*  |
| --- |
|  The [Wishes](http://ww2.wizards.com/Gatherer/index.aspx?term=wish&Field_Name=on&Field_Rules=on&Field_Type=on&setfilter=Judgment) from *Judgment*  |
|  The [Decrees](http://ww2.wizards.com/Gatherer/index.aspx?term=decree&Field_Name=on&Field_Rules=on&Field_Type=on&setfilter=Scourge) from *Scourge*  |
|  The [Pulses](http://ww2.wizards.com/Gatherer/index.aspx?term=pulse&Field_Name=on&Field_Rules=on&Field_Type=on&setfilter=Darksteel) from *Darksteel*  |
|  The [Beacons](http://ww2.wizards.com/Gatherer/index.aspx?term=beacon&Field_Name=on&Field_Rules=on&Field_Type=on&setfilter=Fifth%20Dawn) from *Fifth Dawn*  |
|  The [Shoals](http://ww2.wizards.com/Gatherer/index.aspx?term=shoal&Field_Name=on&Field_Rules=on&Field_Type=on&setfilter=Betrayers%20of%20Kamigawa) from *Betrayers of Kamigawa*  |
|  The [epic spells](http://ww2.wizards.com/Gatherer/index.aspx?term=epic&Field_Name=on&Field_Rules=on&Field_Type=on&setfilter=Saviors%20of%20Kamigawa) from *Saviors of Kamigawa*  |
|  The [Pacts](http://ww2.wizards.com/Gatherer/index.aspx?term=pact&Field_Name=on&Field_Rules=on&Field_Type=on&setfilter=Future%20Sight) from *Future Sight*  |
|  The [Commands](http://ww2.wizards.com/Gatherer/index.aspx?term=command&Field_Name=on&Field_Rules=on&Field_Type=on&setfilter=Lorwyn) from *Lorwyn*  |
|  Some other splashy cycle of rare instants and/or sorceries  |








